(function (global, factory) {
    ...
    function ShowDemoWindow(p_open = null) {
        ...
        if (no_docking.value)
            window_flags |= ImGui.ImGuiWindowFlags.NoDocking;
        if (no_close.value)
...
Without an explicit value, inner_width is == outer_size.x and therefore using Stretch columns + ScrollX together doesn't make sense.`);
    ...
    function ShowExampleAppDockspace(p_open) {
        ...
        const dockspace_flags = STATIC("dockspace_flags#dockspace", ImGui.ImGuiDockNodeFlags.None);
        // We are using the ImGuiWindowFlags_NoDocking flag to make the parent window not dockable into,
        // because it would be confusing to have two docking targets within each others.
        let window_flags = ImGui.ImGuiWindowFlags.MenuBar | ImGui.ImGuiWindowFlags.NoDocking;
        if (opt_fullscreen.value) {
            ...
            ImGui.PushStyleVar(ImGui.ImGuiStyleVar.WindowRounding, 0.0);
            ImGui.PushStyleVar(ImGui.ImGuiStyleVar.WindowBorderSize, 0.0);
            window_flags |= ImGui.ImGuiWindowFlags.NoTitleBar | ImGui.ImGuiWindowFlags.NoCollapse | ImGui.ImGuiWindowFlags.NoResize | ImGui.ImGuiWindowFlags.NoMove;
            window_flags |= ImGui.ImGuiWindowFlags.NoBringToFrontOnFocus | ImGui.ImGuiWindowFlags.NoNavFocus;
        }
        else {
            dockspace_flags.value &= ~ImGui.ImGuiDockNodeFlags.PassthruCentralNode;
        }
        // When using ImGuiDockNodeFlags_PassthruCentralNode, DockSpace() will render our background
        // and handle the pass-thru hole, so we ask Begin() to not render a background.
        if (dockspace_flags.value & ImGui.ImGuiDockNodeFlags.PassthruCentralNode)
            window_flags |= ImGui.ImGuiWindowFlags.NoBackground;
        ...
        if (io.ConfigFlags & ImGui.ConfigFlags.DockingEnable) {
            ...
            ImGui.DockSpace(dockspace_id, new ImGui.ImVec2(0.0, 0.0), dockspace_flags.value);
        }
        else {
        ...
        if (ImGui.BeginMenuBar()) {
            if (ImGui.BeginMenu("Options")) {
                ...
                ImGui.Separator();
                if (ImGui.MenuItem("Flag: NoSplit", "", (dockspace_flags.value & ImGui.ImGuiDockNodeFlags.NoSplit) != 0)) {
                    dockspace_flags.value ^= ImGui.ImGuiDockNodeFlags.NoSplit;
                }
                if (ImGui.MenuItem("Flag: NoResize", "", (dockspace_flags.value & ImGui.ImGuiDockNodeFlags.NoResize) != 0)) {
                    dockspace_flags.value ^= ImGui.ImGuiDockNodeFlags.NoResize;
                }
                if (ImGui.MenuItem("Flag: NoDockingInCentralNode", "", (dockspace_flags.value & ImGui.ImGuiDockNodeFlags.NoDockingInCentralNode) != 0)) {
                    dockspace_flags.value ^= ImGui.ImGuiDockNodeFlags.NoDockingInCentralNode;
                }
                if (ImGui.MenuItem("Flag: AutoHideTabBar", "", (dockspace_flags.value & ImGui.ImGuiDockNodeFlags.AutoHideTabBar) != 0)) {
                    dockspace_flags.value ^= ImGui.ImGuiDockNodeFlags.AutoHideTabBar;
                }
                if (ImGui.MenuItem("Flag: PassthruCentralNode", "", (dockspace_flags.value & ImGui.ImGuiDockNodeFlags.PassthruCentralNode) != 0, opt_fullscreen.value)) {
                    dockspace_flags.value ^= ImGui.ImGuiDockNodeFlags.PassthruCentralNode;
                }
                ImGui.Separator();
