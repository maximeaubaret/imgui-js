...
System.register(["imgui-js"], function (exports_1, context_1) {
    ...
    function ShowDemoWindow(p_open = null) {
        ...
        if (no_titlebar.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoTitleBar;
        if (no_scrollbar.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoScrollbar;
        if (!no_menu.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.MenuBar;
        if (no_move.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoMove;
        if (no_resize.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoResize;
        if (no_collapse.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoCollapse;
        if (no_nav.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoNav;
        if (no_background.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoBackground;
        if (no_bring_to_front.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoBringToFrontOnFocus;
        if (no_docking.value)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoDocking;
        if (no_close.value)
            p_open = null; // Don't pass our bool* to Begin
        // We specify a default position/size in case there's no data in the .ini file. Typically this isn't required! We only do it to make the Demo applications a little more welcoming.
        ImGui.SetNextWindowPos(new imgui_js_25.ImVec2(650, 20), ImGui.Cond.FirstUseEver);
        ImGui.SetNextWindowSize(new imgui_js_25.ImVec2(550, 680), imgui_js_7.ImGuiCond.FirstUseEver);
    ...
    function ShowDemoWindowWidgets() {
        ...
        if (ImGui.TreeNode("Basic")) {
            ...
            for (let i = 0; i < 7; i++) {
                if (i > 0)
                    ImGui.SameLine();
                ImGui.PushID(i);
                ImGui.PushStyleColor(imgui_js_5.ImGuiCol.Button, imgui_js_28.ImColor.HSV(i / 7.0, 0.6, 0.6));
                ImGui.PushStyleColor(imgui_js_5.ImGuiCol.ButtonHovered, imgui_js_28.ImColor.HSV(i / 7.0, 0.7, 0.7));
                ImGui.PushStyleColor(imgui_js_5.ImGuiCol.ButtonActive, imgui_js_28.ImColor.HSV(i / 7.0, 0.8, 0.8));
                ImGui.Button("Click");
            ...
            if (ImGui.ArrowButton("##left", imgui_js_33.ImGuiDir.Left)) {
                counter.value--;
            }
            ImGui.SameLine(0.0, spacing);
            if (ImGui.ArrowButton("##right", imgui_js_33.ImGuiDir.Right)) {
                counter.value++;
            }
        ...
        if (ImGui.TreeNode("Trees")) {
            ...
            if (ImGui.TreeNode("Advanced, with Selectable nodes")) {
                ...
                ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.IndentSpacing, ImGui.GetFontSize() * 3); // Increase spacing to differentiate leaves from expanded contents.
                for (let i = 0; i < 6; i++) {
                    // Disable the default open on single-click behavior and pass in Selected flag according to our selection state.
                    let node_flags = imgui_js_15.ImGuiTreeNodeFlags.OpenOnArrow | imgui_js_15.ImGuiTreeNodeFlags.OpenOnDoubleClick;
                    if (selection_mask.value & (1 << i))
                        node_flags |= imgui_js_15.ImGuiTreeNodeFlags.Selected;
                    if (i < 3) {
                        ...
                        node_flags |= imgui_js_15.ImGuiTreeNodeFlags.Leaf | imgui_js_15.ImGuiTreeNodeFlags.NoTreePushOnOpen; // ImGuiTreeNodeFlags.Bullet
        ...
        if (ImGui.TreeNode("Text")) {
            if (ImGui.TreeNode("Colored Text")) {
                ...
                ImGui.TextColored(new imgui_js_26.ImVec4(1.0, 0.0, 1.0, 1.0), "Pink");
                ImGui.TextColored(new imgui_js_26.ImVec4(1.0, 1.0, 0.0, 1.0), "Yellow");
            ...
            if (ImGui.TreeNode("Word Wrapping")) {
                ...
                ImGui.GetWindowDrawList().AddRectFilled(new imgui_js_25.ImVec2(pos.x + wrap_width.value, pos.y), new imgui_js_25.ImVec2(pos.x + wrap_width.value + 10, pos.y + ImGui.GetTextLineHeight()), imgui_js_27.IM_COL32(255, 0, 255, 255));
                ...
                ImGui.GetWindowDrawList().AddRect(ImGui.GetItemRectMin(), ImGui.GetItemRectMax(), imgui_js_27.IM_COL32(255, 255, 0, 255));
                ImGui.PopTextWrapPos();
        ...
        if (ImGui.TreeNode("Images")) {
            ...
            ImGui.Image(my_tex_id, new imgui_js_25.ImVec2(my_tex_w, my_tex_h), new imgui_js_25.ImVec2(0, 0), new imgui_js_25.ImVec2(1, 1), new imgui_js_26.ImVec4(1.0, 1.0, 1.0, 1.0), new imgui_js_26.ImVec4(1.0, 1.0, 1.0, 0.5));
            if (ImGui.IsItemHovered()) {
                ImGui.BeginTooltip();
                ...
                const uv0 = new imgui_js_25.ImVec2((region_x) / my_tex_w, (region_y) / my_tex_h);
                const uv1 = new imgui_js_25.ImVec2((region_x + region_sz) / my_tex_w, (region_y + region_sz) / my_tex_h);
                ImGui.Image(my_tex_id, new imgui_js_25.ImVec2(region_sz * zoom, region_sz * zoom), uv0, uv1, new imgui_js_28.ImColor(255, 255, 255, 255).toImVec4(), new imgui_js_28.ImColor(255, 255, 255, 128).toImVec4());
                ImGui.EndTooltip();
            }
            ...
            for (let i = 0; i < 8; i++) {
                ...
                if (ImGui.ImageButton(my_tex_id, new imgui_js_25.ImVec2(32, 32), new imgui_js_25.ImVec2(0, 0), new imgui_js_25.ImVec2(32.0 / my_tex_w, 32 / my_tex_h), frame_padding, new imgui_js_26.ImVec4(0, 0, 0, 1)))
        ...
        if (ImGui.TreeNode("Selectables")) {
            ...
            if (ImGui.TreeNode("Basic")) {
                ...
                if (ImGui.Selectable("5. I am double clickable", selection.value[4], imgui_js_13.ImGuiSelectableFlags.AllowDoubleClick))
            ...
            if (ImGui.TreeNode("Grid")) {
                ...
                for (let i = 0; i < 4 * 4; i++) {
                    ImGui.PushID(i);
                    if (ImGui.Selectable("Sailor", (value = selected.value[i]) => selected.value[i] = value, 0, new imgui_js_25.ImVec2(50, 50))) {
            ...
            if (ImGui.TreeNode("Alignment")) {
                ...
                for (let y = 0; y < 3; y++) {
                    for (let x = 0; x < 3; x++) {
                        const alignment = new imgui_js_25.ImVec2(x / 2.0, y / 2.0);
                        // char name[32];
                        // sprintf(name, "(%.1f,%.1f)", alignment.x, alignment.y);
                        const name = `(${alignment.x.toFixed(1)},${alignment.y.toFixed(1)})`;
                        if (x > 0)
                            ImGui.SameLine();
                        ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.SelectableTextAlign, alignment);
                        // ImGui.Selectable(name, &selected[3*y+x], ImGuiSelectableFlags_None, ImVec2(80,80));
                        ImGui.Selectable(name, (value = selected.value[3 * y + x]) => selected.value[3 * y + x] = value, imgui_js_13.ImGuiSelectableFlags.None, new imgui_js_25.ImVec2(80, 80));
                        ImGui.PopStyleVar();
                    }
                }
                ImGui.TreePop();
        ...
        if (ImGui.TreeNode("Text Input")) {
            if (ImGui.TreeNode("Multi-line Text Input")) {
                ...
                /* static */ const text = STATIC("text", new imgui_js_4.ImStringBuffer(1024 * 16, "/*\n" +
                    ...
                    "label:\n" +
                    "\tlock cmpxchg8b eax\n"));
                /* static */ const flags = STATIC("flags", imgui_js_11.ImGuiInputTextFlags.AllowTabInput);
                ...
                ImGui.CheckboxFlags("ImGuiInputTextFlags_ReadOnly", (value = flags.value) => flags.value = value, imgui_js_11.ImGuiInputTextFlags.ReadOnly);
                ImGui.CheckboxFlags("ImGuiInputTextFlags_AllowTabInput", (value = flags.value) => flags.value = value, imgui_js_11.ImGuiInputTextFlags.AllowTabInput);
                ImGui.CheckboxFlags("ImGuiInputTextFlags_CtrlEnterForNewLine", (value = flags.value) => flags.value = value, imgui_js_11.ImGuiInputTextFlags.CtrlEnterForNewLine);
                ImGui.InputTextMultiline("##source", text.value, imgui_js_3.IM_ARRAYSIZE(text.value), new imgui_js_25.ImVec2(-1.0, ImGui.GetTextLineHeight() * 16), flags.value);
                ImGui.TreePop();
            }
            if (ImGui.TreeNode("Filtered Text Input")) {
                ...
                ImGui.InputText("decimal", buf2.value, imgui_js_3.IM_ARRAYSIZE(buf2.value), imgui_js_11.ImGuiInputTextFlags.CharsDecimal);
                /* static */ const buf3 = STATIC("buf3", new imgui_js_4.ImStringBuffer(64, ""));
                ImGui.InputText("hexadecimal", buf3.value, imgui_js_3.IM_ARRAYSIZE(buf3.value), imgui_js_11.ImGuiInputTextFlags.CharsHexadecimal | imgui_js_11.ImGuiInputTextFlags.CharsUppercase);
                /* static */ const buf4 = STATIC("buf4", new imgui_js_4.ImStringBuffer(64, ""));
                ImGui.InputText("uppercase", buf4.value, imgui_js_3.IM_ARRAYSIZE(buf4.value), imgui_js_11.ImGuiInputTextFlags.CharsUppercase);
                /* static */ const buf5 = STATIC("buf5", new imgui_js_4.ImStringBuffer(64, ""));
                ImGui.InputText("no blank", buf5.value, imgui_js_3.IM_ARRAYSIZE(buf5.value), imgui_js_11.ImGuiInputTextFlags.CharsNoBlank);
                class TextFilters {
                ...
                ImGui.InputText("\"imgui\" letters", buf6.value, imgui_js_3.IM_ARRAYSIZE(buf6.value), imgui_js_11.ImGuiInputTextFlags.CallbackCharFilter, TextFilters.FilterImGuiLetters);
                ImGui.Text("Password input");
                /* static */ const bufpass = STATIC("bufpass", new imgui_js_4.ImStringBuffer(64, "password123"));
                ImGui.InputText("password", bufpass.value, imgui_js_3.IM_ARRAYSIZE(bufpass.value), imgui_js_11.ImGuiInputTextFlags.Password | imgui_js_11.ImGuiInputTextFlags.CharsNoBlank);
                ImGui.SameLine();
                HelpMarker("Display all characters as '*'.\nDisable clipboard cut and copy.\nDisable logging.\n");
                ImGui.InputText("password (clear)", bufpass.value, imgui_js_3.IM_ARRAYSIZE(bufpass.value), imgui_js_11.ImGuiInputTextFlags.CharsNoBlank);
                ImGui.TreePop();
            }
        ...
        if (ImGui.TreeNode("Plots Widgets")) {
            ...
            ImGui.PlotLines("Lines", values.value, imgui_js_3.IM_ARRAYSIZE(values.value), values_offset.value, "avg 0.0", -1.0, 1.0, new imgui_js_25.ImVec2(0, 80));
            ImGui.PlotHistogram("Histogram", arr.value, imgui_js_3.IM_ARRAYSIZE(arr.value), 0, null, 0.0, 1.0, new imgui_js_25.ImVec2(0, 80));
            ...
            ImGui.PlotLines("Lines", func, null, display_count.value, 0, null, -1.0, 1.0, new imgui_js_25.ImVec2(0, 80));
            ImGui.PlotHistogram("Histogram", func, null, display_count.value, 0, null, -1.0, 1.0, new imgui_js_25.ImVec2(0, 80));
            ImGui.Separator();
            ...
            ImGui.ProgressBar(progress.value, new imgui_js_25.ImVec2(0.0, 0.0));
            ...
            ImGui.ProgressBar(progress.value, new imgui_js_25.ImVec2(0., 0.), buf);
            ImGui.TreePop();
        }
        if (ImGui.TreeNode("Color/Picker Widgets")) {
            /* static */ const color = STATIC("color#863", new imgui_js_26.ImVec4(114.0 / 255.0, 144.0 / 255.0, 154.0 / 255.0, 200.0 / 255.0));
            ...
            if (saved_palette_init.value) {
                for (let n = 0; n < 32; n++) {
                    saved_palette.value[n] = new imgui_js_26.ImVec4();
            ...
            /* static */ const backup_color = STATIC("backup_color", new imgui_js_26.ImVec4());
            ...
            if (ImGui.BeginPopup("mypicker")) {
                ...
                ImGui.Text("Current");
                ImGui.ColorButton("##current", color.value, imgui_js_6.ImGuiColorEditFlags.NoPicker | imgui_js_6.ImGuiColorEditFlags.AlphaPreviewHalf, new imgui_js_25.ImVec2(60, 40));
                ImGui.Text("Previous");
                if (ImGui.ColorButton("##previous", backup_color.value, imgui_js_6.ImGuiColorEditFlags.NoPicker | imgui_js_6.ImGuiColorEditFlags.AlphaPreviewHalf, new imgui_js_25.ImVec2(60, 40)))
                ...
                for (let n = 0; n < imgui_js_3.IM_ARRAYSIZE(saved_palette.value); n++) {
                    ...
                    if (ImGui.ColorButton("##palette", saved_palette.value[n], imgui_js_6.ImGuiColorEditFlags.NoAlpha | imgui_js_6.ImGuiColorEditFlags.NoPicker | imgui_js_6.ImGuiColorEditFlags.NoTooltip, new imgui_js_25.ImVec2(20, 20)))
                        color.value.Copy(new imgui_js_26.ImVec4(saved_palette.value[n].x, saved_palette.value[n].y, saved_palette.value[n].z, color.value.w)); // Preserve alpha!
            ...
            ImGui.ColorButton("MyColor##3c", color.value, misc_flags, new imgui_js_25.ImVec2(80, 80));
            ...
            /* static */ const ref_color_v = STATIC("ref_color_v", new imgui_js_26.ImVec4(1.0, 0.0, 1.0, 0.5));
            ...
            /* static */ const color_stored_as_hsv = STATIC("color_stored_as_hsv", new imgui_js_26.ImVec4(0.23, 1.0, 1.0, 1.0));
            ImGui.Spacing();
        ...
        if (ImGui.TreeNode("Data Types")) {
            ...
            ImGui.InputScalar("input s8 hex", s8_v.value, inputs_step.value ? s8_one : null, null, "%08X", imgui_js_11.ImGuiInputTextFlags.CharsHexadecimal);
            ImGui.InputScalar("input s16", s16_v.value, inputs_step.value ? s16_one : null, null, "%d");
            ImGui.InputScalar("input s16 hex", s16_v.value, inputs_step.value ? s16_one : null, null, "%08X", imgui_js_11.ImGuiInputTextFlags.CharsHexadecimal);
            ImGui.InputScalar("input s32", s32_v.value, inputs_step.value ? s32_one : null, null, "%d");
            ImGui.InputScalar("input s32 hex", s32_v.value, inputs_step.value ? s32_one : null, null, "%08X", imgui_js_11.ImGuiInputTextFlags.CharsHexadecimal);
            ImGui.InputScalar("input u32", u32_v.value, inputs_step.value ? u32_one : null, null, "%u");
            ImGui.InputScalar("input u32 hex", u32_v.value, inputs_step.value ? u32_one : null, null, "%08X", imgui_js_11.ImGuiInputTextFlags.CharsHexadecimal);
        ...
        if (ImGui.TreeNode("Vertical Sliders")) {
            const spacing = 4;
            ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.ItemSpacing, new imgui_js_25.ImVec2(spacing, spacing));
            /* static */ const int_value = STATIC("int_value", 0);
            ImGui.VSliderInt("##int", new imgui_js_25.ImVec2(18, 160), (value = int_value.value) => int_value.value = value, 0, 5);
            ImGui.SameLine();
            ...
            for (let i = 0; i < 7; i++) {
                if (i > 0)
                    ImGui.SameLine();
                ImGui.PushID(i);
                ImGui.PushStyleColor(imgui_js_5.ImGuiCol.FrameBg, imgui_js_28.ImColor.HSV(i / 7.0, 0.5, 0.5));
                ImGui.PushStyleColor(imgui_js_5.ImGuiCol.FrameBgHovered, imgui_js_28.ImColor.HSV(i / 7.0, 0.6, 0.5));
                ImGui.PushStyleColor(imgui_js_5.ImGuiCol.FrameBgActive, imgui_js_28.ImColor.HSV(i / 7.0, 0.7, 0.5));
                ImGui.PushStyleColor(imgui_js_5.ImGuiCol.SliderGrab, imgui_js_28.ImColor.HSV(i / 7.0, 0.9, 0.9));
                ImGui.VSliderFloat("##v", new imgui_js_25.ImVec2(18, 160), (value = values.value[i]) => values.value[i] = value, 0.0, 1.0, "");
            ...
            const rows = 3;
            const small_slider_size = new imgui_js_25.ImVec2(18, (160.0 - (rows - 1) * spacing) / rows);
            ...
            for (let i = 0; i < 4; i++) {
                if (i > 0)
                    ImGui.SameLine();
                ImGui.PushID(i);
                ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.GrabMinSize, 40);
                ImGui.VSliderFloat("##v", new imgui_js_25.ImVec2(40, 160), (value = values.value[i]) => values.value[i] = value, 0.0, 1.0, "%.2f\nsec");
                ImGui.PopStyleVar();
                ImGui.PopID();
            }
            ImGui.PopID();
        ...
        if (ImGui.TreeNode("Drag and Drop")) {
            ...
            }
            {
                ...
                for (let n = 0; n < imgui_js_3.IM_ARRAYSIZE(names.value); n++) {
                    ...
                    if ((n % 3) != 0)
                        ImGui.SameLine();
                    ImGui.Button(names.value[n], new imgui_js_25.ImVec2(60, 60));
        ...
        if (ImGui.TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
            ...
            ImGui.BulletText(`Return value = ${ret}\n` +
                ...
                `IsItemHovered(_AllowWhenBlockedByPopup) = ${ImGui.IsItemHovered(imgui_js_10.ImGuiHoveredFlags.AllowWhenBlockedByPopup)}\n` +
                `IsItemHovered(_AllowWhenBlockedByActiveItem) = ${ImGui.IsItemHovered(imgui_js_10.ImGuiHoveredFlags.AllowWhenBlockedByActiveItem)}\n` +
                `IsItemHovered(_AllowWhenOverlapped) = ${ImGui.IsItemHovered(imgui_js_10.ImGuiHoveredFlags.AllowWhenOverlapped)}\n` +
                `IsItemhovered(_RectOnly) = ${ImGui.IsItemHovered(imgui_js_10.ImGuiHoveredFlags.RectOnly)}\n` +
            ...
            if (embed_all_inside_a_child_window.value)
                ImGui.BeginChild("outer_child", new imgui_js_25.ImVec2(0, ImGui.GetFontSize() * 20), true);
            ...
            ImGui.BulletText(`IsWindowHovered() = ${ImGui.IsWindowHovered()}\n` +
                `IsWindowHovered(_AllowWhenBlockedByPopup) = ${ImGui.IsWindowHovered(imgui_js_10.ImGuiHoveredFlags.AllowWhenBlockedByPopup)}\n` +
                `IsWindowHovered(_AllowWhenBlockedByActiveItem) = ${ImGui.IsWindowHovered(imgui_js_10.ImGuiHoveredFlags.AllowWhenBlockedByActiveItem)}\n` +
                `IsWindowHovered(_ChildWindows) = ${ImGui.IsWindowHovered(imgui_js_10.ImGuiHoveredFlags.ChildWindows)}\n` +
                `IsWindowHovered(_ChildWindows|_RootWindow) = ${ImGui.IsWindowHovered(imgui_js_10.ImGuiHoveredFlags.ChildWindows | imgui_js_10.ImGuiHoveredFlags.RootWindow)}\n` +
                `IsWindowFocused(_ChildWindows|_AllowWhenBlockedByPopup) = ${ImGui.IsWindowFocused(imgui_js_10.ImGuiHoveredFlags.ChildWindows | imgui_js_10.ImGuiHoveredFlags.AllowWhenBlockedByPopup)}\n` +
                `IsWindowHovered(_RootWindow) = ${ImGui.IsWindowHovered(imgui_js_10.ImGuiHoveredFlags.RootWindow)}\n` +
                `IsWindowHovered(_AnyWindow) = ${ImGui.IsWindowHovered(imgui_js_10.ImGuiHoveredFlags.AnyWindow)}\n`);
            ImGui.BeginChild("child", new imgui_js_25.ImVec2(0, 50), true);
    ...
    function ShowDemoWindowLayout() {
        ...
        if (ImGui.TreeNode("Child windows")) {
            ...
            ImGui.SameLine();
            ImGui.SetNextItemWidth(100);
            goto_line = ImGui.InputInt("##Line", (value = line.value) => line.value = value, 0, 0, imgui_js_11.ImGuiInputTextFlags.EnterReturnsTrue) || goto_line;
            // Child 1: no border, enable horizontal scrollbar
            {
                const window_flags = imgui_js_16.ImGuiWindowFlags.HorizontalScrollbar | (disable_mouse_wheel.value ? imgui_js_16.ImGuiWindowFlags.NoScrollWithMouse : 0);
                ImGui.BeginChild("Child1", new imgui_js_25.ImVec2(ImGui.GetWindowContentRegionWidth() * 0.5, 260), false, window_flags);
                for (let i = 0; i < 100; i++) {
            ...
            {
                const window_flags = (disable_mouse_wheel.value ? imgui_js_16.ImGuiWindowFlags.NoScrollWithMouse : 0) | (disable_menu.value ? 0 : imgui_js_16.ImGuiWindowFlags.MenuBar);
                ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.ChildRounding, 5.0);
                ImGui.BeginChild("Child2", new imgui_js_25.ImVec2(0, 260), true, window_flags);
                ...
                for (let i = 0; i < 100; i++) {
                    ...
                    ImGui.Button(buf, new imgui_js_25.ImVec2(-1.0, 0.0));
                    ImGui.NextColumn();
                }
                ImGui.EndChild();
                ImGui.PopStyleVar();
            ...
            //   See "Widgets" -> "Querying Status (Active/Focused/Hovered etc.)" section for more details about this.
            {
                ...
                ImGui.PushStyleColor(imgui_js_5.ImGuiCol.ChildBg, imgui_js_27.IM_COL32(255, 0, 0, 100));
                ImGui.BeginChild("blah", new imgui_js_25.ImVec2(200, 100), true, imgui_js_16.ImGuiWindowFlags.None);
                for (let n = 0; n < 50; n++)
        ...
        if (ImGui.TreeNode("Basic Horizontal Layout")) {
            ...
            ImGui.SameLine();
            ImGui.TextColored(new imgui_js_26.ImVec4(1, 1, 0, 1), "Sailor");
            // Adjust spacing
            ...
            ImGui.PopItemWidth();
            // Dummy
            const button_sz = new imgui_js_25.ImVec2(40, 40);
        ...
        if (ImGui.TreeNode("Tabs")) {
            if (ImGui.TreeNode("Basic")) {
                const tab_bar_flags = imgui_js_22.ImGuiTabBarFlags.None;
            ...
            if (ImGui.TreeNode("Advanced & Close Button")) {
                ...
                /* static */ const tab_bar_flags = STATIC("tab_bar_flags", imgui_js_22.ImGuiTabBarFlags.Reorderable);
                ...
                ImGui.CheckboxFlags("ImGuiTabBarFlags_TabListPopupButton", (value = tab_bar_flags.value) => tab_bar_flags.value = value, imgui_js_22.ImGuiTabBarFlags.TabListPopupButton);
                ...
                if ((tab_bar_flags.value & imgui_js_22.ImGuiTabBarFlags.FittingPolicyMask_) === 0)
                    tab_bar_flags.value |= imgui_js_22.ImGuiTabBarFlags.FittingPolicyDefault_;
                if (ImGui.CheckboxFlags("ImGuiTabBarFlags_FittingPolicyResizeDown", (value = tab_bar_flags.value) => tab_bar_flags.value = value, imgui_js_22.ImGuiTabBarFlags.FittingPolicyResizeDown))
                    tab_bar_flags.value &= ~(imgui_js_22.ImGuiTabBarFlags.FittingPolicyMask_ ^ imgui_js_22.ImGuiTabBarFlags.FittingPolicyResizeDown);
                if (ImGui.CheckboxFlags("ImGuiTabBarFlags_FittingPolicyScroll", (value = tab_bar_flags.value) => tab_bar_flags.value = value, imgui_js_22.ImGuiTabBarFlags.FittingPolicyScroll))
                    tab_bar_flags.value &= ~(imgui_js_22.ImGuiTabBarFlags.FittingPolicyMask_ ^ imgui_js_22.ImGuiTabBarFlags.FittingPolicyScroll);
                // Tab Bar
        ...
        if (ImGui.TreeNode("Groups")) {
            ...
            ImGui.Button("ACTION", new imgui_js_25.ImVec2((size.x - ImGui.GetStyle().ItemSpacing.x) * 0.5, size.y));
            ImGui.SameLine();
            ImGui.Button("REACTION", new imgui_js_25.ImVec2((size.x - ImGui.GetStyle().ItemSpacing.x) * 0.5, size.y));
            ImGui.EndGroup();
            ImGui.SameLine();
        ...
        if (ImGui.TreeNode("Scrolling")) {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                const child_flags = imgui_js_16.ImGuiWindowFlags.MenuBar;
                ImGui.BeginChild(ImGui.GetID(i), new imgui_js_25.ImVec2(child_w, 200.0), true, child_flags);
                if (scroll_to_off)
                ...
                for (let line = 0; line < 100; line++) {
                    if (track.value && line === track_line.value) {
                        ImGui.TextColored(new imgui_js_26.ImVec4(1, 1, 0, 1), `Line ${line}`);
        ...
        if (ImGui.TreeNode("Horizontal Scrolling")) {
            ...
            ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.FrameRounding, 3.0);
            ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.FramePadding, new imgui_js_25.ImVec2(2.0, 1.0));
            ImGui.BeginChild("scrolling", new imgui_js_25.ImVec2(0, ImGui.GetFrameHeightWithSpacing() * 7 + 30), true, imgui_js_16.ImGuiWindowFlags.HorizontalScrollbar);
            for (let line = 0; line < lines.value; line++) {
                ...
                for (let n = 0; n < num_buttons; n++) {
                    ...
                    const hue = n * 0.05;
                    ImGui.PushStyleColor(imgui_js_5.ImGuiCol.Button, imgui_js_28.ImColor.HSV(hue, 0.6, 0.6));
                    ImGui.PushStyleColor(imgui_js_5.ImGuiCol.ButtonHovered, imgui_js_28.ImColor.HSV(hue, 0.7, 0.7));
                    ImGui.PushStyleColor(imgui_js_5.ImGuiCol.ButtonActive, imgui_js_28.ImColor.HSV(hue, 0.8, 0.8));
                    ImGui.Button(label, new imgui_js_25.ImVec2(40.0 + Math.sin(line + n) * 20.0, 0.0));
                    ImGui.PopStyleColor(3);
                    ImGui.PopID();
                }
            }
        ...
        if (ImGui.TreeNode("Clipping")) {
            /* static */ const size = STATIC("size", new imgui_js_25.ImVec2(100, 100)), offset = STATIC("offset", new imgui_js_25.ImVec2(50, 20));
            ImGui.TextWrapped("On a per-widget basis we are occasionally clipping text CPU-side if it won't fit in its frame. Otherwise we are doing coarser clipping + passing a scissor rectangle to the renderer. The system is designed to try minimizing both execution and CPU/GPU rendering cost.");
            ImGui.DragFloat2("size", size.value, 0.5, 1.0, 200.0, "%.0f");
            ImGui.TextWrapped("(Click and drag)");
            const pos = ImGui.GetCursorScreenPos();
            const clip_rect = new imgui_js_26.ImVec4(pos.x, pos.y, pos.x + size.value.x, pos.y + size.value.y);
            ...
            ImGui.GetWindowDrawList().AddRectFilled(pos, new imgui_js_25.ImVec2(pos.x + size.value.x, pos.y + size.value.y), imgui_js_27.IM_COL32(90, 90, 120, 255));
            ImGui.GetWindowDrawList().AddText(ImGui.GetFont(), ImGui.GetFontSize() * 2.0, new imgui_js_25.ImVec2(pos.x + offset.value.x, pos.y + offset.value.y), imgui_js_27.IM_COL32(255, 255, 255, 255), "Line 1 hello\nLine 2 clip me!", null, 0.0, clip_rect);
            ImGui.TreePop();
        }
    }
    function ShowDemoWindowPopups() {
        ...
        if (ImGui.TreeNode("Modals")) {
            ...
            if (ImGui.BeginPopupModal("Delete?", null, imgui_js_16.ImGuiWindowFlags.AlwaysAutoResize)) {
                ...
                ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.FramePadding, new imgui_js_25.ImVec2(0, 0));
                ImGui.Checkbox("Don't ask me next time", (value = dont_ask_me_next_time.value) => dont_ask_me_next_time.value = value);
                ImGui.PopStyleVar();
                if (ImGui.Button("OK", new imgui_js_25.ImVec2(120, 0))) {
                    ImGui.CloseCurrentPopup();
                }
                ImGui.SetItemDefaultFocus();
                ImGui.SameLine();
                if (ImGui.Button("Cancel", new imgui_js_25.ImVec2(120, 0))) {
                    ImGui.CloseCurrentPopup();
                }
                ImGui.EndPopup();
            }
            ...
            if (ImGui.BeginPopupModal("Stacked 1", null, imgui_js_16.ImGuiWindowFlags.MenuBar)) {
                if (ImGui.BeginMenuBar()) {
    ...
    function ShowDemoWindowColumns() {
        ...
        if (disable_indent.value)
            ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.IndentSpacing, 0.0);
        // Basic columns
        if (ImGui.TreeNode("Basic")) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (ImGui.Selectable(label, selected.value === i, imgui_js_13.ImGuiSelectableFlags.SpanAllColumns))
                    selected.value = i;
        ...
        if (ImGui.TreeNode("Borders")) {
            ...
            for (let i = 0; i < 4 * 3; i++) {
                ...
                ImGui.Button("Button", new imgui_js_25.ImVec2(-1.0, 0.0));
                ImGui.NextColumn();
            }
            ImGui.Columns(1);
            if (h_borders.value)
        ...
        */
        if (ImGui.TreeNode("Horizontal Scrolling")) {
            ImGui.SetNextWindowContentSize(new imgui_js_25.ImVec2(1500.0, 0.0));
            ImGui.BeginChild("##ScrollingRegion", new imgui_js_25.ImVec2(0, ImGui.GetFontSize() * 20), false, imgui_js_16.ImGuiWindowFlags.HorizontalScrollbar);
            ImGui.Columns(10);
            const ITEMS_COUNT = 2000;
            const clipper = new imgui_js_32.ImGuiListClipper(); // Also demonstrate using the clipper for large list
    ...
    function PushStyleCompact() {
        const style = ImGui.GetStyle();
        ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.FramePadding, new imgui_js_25.ImVec2(style.FramePadding.x, Math.floor(style.FramePadding.y * 0.60)));
        ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.ItemSpacing, new imgui_js_25.ImVec2(style.ItemSpacing.x, Math.floor(style.ItemSpacing.y * 0.60)));
    }
    ...
    function EditTableSizingFlags(p_flags) {
        let policies = [
            { value: imgui_js_18.ImGuiTableFlags.None, name: "Default", tooltip: "Use default sizing policy:\n- ImGuiTableFlags_SizingFixedFit if ScrollX is on or if host window has ImGuiWindowFlags_AlwaysAutoResize.\n- ImGuiTableFlags_SizingStretchSame otherwise." },
            { value: imgui_js_18.ImGuiTableFlags.SizingFixedFit, name: "ImGuiTableFlags_SizingFixedFit", tooltip: "Columns default to _WidthFixed (if resizable) or _WidthAuto (if not resizable), matching contents width." },
            { value: imgui_js_18.ImGuiTableFlags.SizingFixedSame, name: "ImGuiTableFlags_SizingFixedSame", tooltip: "Columns are all the same width, matching the maximum contents width.\nImplicitly disable ImGuiTableFlags_Resizable and enable ImGuiTableFlags_NoKeepColumnsVisible." },
            { value: imgui_js_18.ImGuiTableFlags.SizingStretchProp, name: "ImGuiTableFlags_SizingStretchProp", tooltip: "Columns default to _WidthStretch with weights proportional to their widths." },
            { value: imgui_js_18.ImGuiTableFlags.SizingStretchSame, name: "ImGuiTableFlags_SizingStretchSame", tooltip: "Columns default to _WidthStretch with same weights." }
        ];
        let idx = 0;
        for (idx = 0; idx < imgui_js_3.IM_ARRAYSIZE(policies); idx++)
            if (policies[idx].value == (p_flags.value & imgui_js_18.ImGuiTableFlags.SizingMask_))
                break;
        ...
        if (ImGui.BeginCombo("Sizing Policy", preview_text)) {
            for (let n = 0; n < imgui_js_3.IM_ARRAYSIZE(policies); n++)
                if (ImGui.Selectable(policies[n].name, idx == n))
                    p_flags.value = (p_flags.value & ~imgui_js_18.ImGuiTableFlags.SizingMask_) | policies[n].value;
            ImGui.EndCombo();
        }
        ImGui.SameLine();
        ImGui.TextDisabled("(?)");
    ...
    function EditTableColumnsFlags(p_flags) {
        ImGui.CheckboxFlags("_DefaultHide", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.DefaultHide);
        ImGui.CheckboxFlags("_DefaultSort", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.DefaultSort);
        if (ImGui.CheckboxFlags("_WidthStretch", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.WidthStretch))
            p_flags.value &= ~(imgui_js_19.ImGuiTableColumnFlags.WidthMask_ ^ imgui_js_19.ImGuiTableColumnFlags.WidthStretch);
        if (ImGui.CheckboxFlags("_WidthFixed", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.WidthFixed))
            p_flags.value &= ~(imgui_js_19.ImGuiTableColumnFlags.WidthMask_ ^ imgui_js_19.ImGuiTableColumnFlags.WidthFixed);
        ImGui.CheckboxFlags("_NoResize", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.NoResize);
        ImGui.CheckboxFlags("_NoReorder", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.NoReorder);
        ImGui.CheckboxFlags("_NoHide", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.NoHide);
        ImGui.CheckboxFlags("_NoClip", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.NoClip);
        ImGui.CheckboxFlags("_NoSort", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.NoSort);
        ImGui.CheckboxFlags("_NoSortAscending", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.NoSortAscending);
        ImGui.CheckboxFlags("_NoSortDescending", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.NoSortDescending);
        ImGui.CheckboxFlags("_NoHeaderWidth", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.NoHeaderWidth);
        ImGui.CheckboxFlags("_PreferSortAscending", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.PreferSortAscending);
        ImGui.CheckboxFlags("_PreferSortDescending", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.PreferSortDescending);
        ImGui.CheckboxFlags("_IndentEnable", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.IndentEnable);
        ImGui.SameLine();
        HelpMarker("Default for column 0");
        ImGui.CheckboxFlags("_IndentDisable", (value = p_flags.value) => p_flags.value = value, imgui_js_19.ImGuiTableColumnFlags.IndentDisable);
        ImGui.SameLine();
        HelpMarker("Default for column >0");
    }
    function ShowTableColumnsStatusFlags(flags) {
        const flags_s = [flags];
        ImGui.CheckboxFlags("_IsEnabled", flags_s, imgui_js_19.ImGuiTableColumnFlags.IsEnabled);
        ImGui.CheckboxFlags("_IsVisible", flags_s, imgui_js_19.ImGuiTableColumnFlags.IsVisible);
        ImGui.CheckboxFlags("_IsSorted", flags_s, imgui_js_19.ImGuiTableColumnFlags.IsSorted);
        ImGui.CheckboxFlags("_IsHovered", flags_s, imgui_js_19.ImGuiTableColumnFlags.IsHovered);
    }
...
This is also more similar to the old NextColumn() function of the Columns API, and provided to facilitate the Columns->Tables API transition.`);
        ...
        if (ImGui.TreeNode("Borders, background")) {
            ...
            ;
            /* static */ const flags = STATIC("flags#tables2", imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.RowBg);
            ...
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags.RowBg", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.RowBg);
            ImGui.CheckboxFlags("ImGuiTableFlags.Borders", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Borders);
            ImGui.SameLine();
            HelpMarker("ImGuiTableFlags.Borders\n = ImGuiTableFlags.BordersInnerV\n | ImGuiTableFlags.BordersOuterV\n | ImGuiTableFlags.BordersInnerV\n | ImGuiTableFlags.BordersOuterH");
            ImGui.Indent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersH", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersH);
            ImGui.Indent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersOuterH", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersOuterH);
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersInnerH", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersInnerH);
            ImGui.Unindent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersV", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersV);
            ImGui.Indent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersOuterV", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersOuterV);
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersInnerV", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersInnerV);
            ImGui.Unindent();
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersOuter", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersOuter);
            ImGui.CheckboxFlags("ImGuiTableFlags.BordersInner", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersInner);
            ImGui.Unindent();
            ImGui.AlignTextToFramePadding();
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags.NoBordersInBody", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoBordersInBody);
            ImGui.SameLine();
                ...
                for (let row = 0; row < 5; row++) {
                    ...
                    for (let column = 0; column < 3; column++) {
                        ...
                        if (contents_type.value == ContentsType.CT_Text)
                            ...
                            ImGui.Button(`Hello ${column},${row}`, new imgui_js_25.ImVec2(-1.0, 0.0));
                    }
                }
                ImGui.EndTable();
            }
        ...
        if (ImGui.TreeNode("Resizable, stretch")) {
            ...
            /* static */ const flags = STATIC("flags#tables3", imgui_js_18.ImGuiTableFlags.SizingStretchSame | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.BordersOuter | imgui_js_18.ImGuiTableFlags.BordersV | imgui_js_18.ImGuiTableFlags.ContextMenuInBody);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersV", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersV);
            ImGui.SameLine();
...
Double-click a column border to auto-fit the column to its contents.`);
            PushStyleCompact();
            /* static */ const flags = STATIC("flags#tables4", imgui_js_18.ImGuiTableFlags.SizingFixedFit | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.BordersOuter | imgui_js_18.ImGuiTableFlags.BordersV | imgui_js_18.ImGuiTableFlags.ContextMenuInBody);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendX", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoHostExtendX);
            PopStyleCompact();
            if (ImGui.BeginTable("table1", 3, flags.value)) {
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
...
When combining Fixed and Stretch columns, generally you only want one, maybe two trailing columns to use _WidthStretch.`);
            const flags = imgui_js_18.ImGuiTableFlags.SizingFixedFit | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable;
            if (ImGui.BeginTable("table1", 3, flags)) {
                ImGui.TableSetupColumn("AAA", imgui_js_19.ImGuiTableColumnFlags.WidthFixed);
                ImGui.TableSetupColumn("BBB", imgui_js_19.ImGuiTableColumnFlags.WidthFixed);
                ImGui.TableSetupColumn("CCC", imgui_js_19.ImGuiTableColumnFlags.WidthStretch);
                ImGui.TableHeadersRow();
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
                    for (let column = 0; column < 3; column++) {
            ...
            if (ImGui.BeginTable("table2", 6, flags)) {
                ImGui.TableSetupColumn("AAA", imgui_js_19.ImGuiTableColumnFlags.WidthFixed);
                ImGui.TableSetupColumn("BBB", imgui_js_19.ImGuiTableColumnFlags.WidthFixed);
                ImGui.TableSetupColumn("CCC", imgui_js_19.ImGuiTableColumnFlags.WidthFixed | imgui_js_19.ImGuiTableColumnFlags.DefaultHide);
                ImGui.TableSetupColumn("DDD", imgui_js_19.ImGuiTableColumnFlags.WidthStretch);
                ImGui.TableSetupColumn("EEE", imgui_js_19.ImGuiTableColumnFlags.WidthStretch);
                ImGui.TableSetupColumn("FFF", imgui_js_19.ImGuiTableColumnFlags.WidthStretch | imgui_js_19.ImGuiTableColumnFlags.DefaultHide);
                ImGui.TableHeadersRow();
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
...
Right-click on a header to open a context menu.`);
            /* static */ const flags = STATIC("flags#tables-reorderable", imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable | imgui_js_18.ImGuiTableFlags.BordersOuter | imgui_js_18.ImGuiTableFlags.BordersV);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_Reorderable", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Reorderable);
            ImGui.CheckboxFlags("ImGuiTableFlags_Hideable", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Hideable);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoBordersInBody", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoBordersInBody);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoBordersInBodyUntilResize", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoBordersInBodyUntilResize);
            ImGui.SameLine();
            ...
            if (ImGui.BeginTable("table2", 3, flags.value | imgui_js_18.ImGuiTableFlags.SizingFixedFit, new imgui_js_25.ImVec2(0.0, 0.0))) {
                ImGui.TableSetupColumn("One");
                ImGui.TableSetupColumn("Two");
                ImGui.TableSetupColumn("Three");
                ImGui.TableHeadersRow();
...
In this demo we don't show horizontal borders to emphasis how they don't affect default horizontal padding.`);
            /* static */ const flags1 = STATIC("flags1#tables-padding", imgui_js_18.ImGuiTableFlags.BordersV);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_PadOuterX", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.PadOuterX);
            ImGui.SameLine();
            HelpMarker("Enable outer-most padding (default if ImGuiTableFlags_BordersOuterV is set)");
            ImGui.CheckboxFlags("ImGuiTableFlags_NoPadOuterX", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.NoPadOuterX);
            ImGui.SameLine();
            HelpMarker("Disable outer-most padding (default if ImGuiTableFlags_BordersOuterV is not set)");
            ImGui.CheckboxFlags("ImGuiTableFlags_NoPadInnerX", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.NoPadInnerX);
            ImGui.SameLine();
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersOuterV", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.BordersOuterV);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersInnerV", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.BordersInnerV);
            ...
            /* static */ const flags2 = STATIC("flags2#tables-padding", imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.RowBg);
            /* static */ const cell_padding = STATIC("cell_padding#tables-padding", new imgui_js_25.ImVec2(0.0, 0.0));
            /* static */ const show_widget_frame_bg = STATIC("const show_widget_frame_bg#tables-padding", true);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Borders", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.Borders);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersH", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.BordersH);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersV", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.BordersV);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersInner", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.BordersInner);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersOuter", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.BordersOuter);
            ImGui.CheckboxFlags("ImGuiTableFlags_RowBg", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.RowBg);
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.Resizable);
            ...
            PopStyleCompact();
            ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.CellPadding, cell_padding.value);
        ...
        if (ImGui.TreeNode("Sizing policies")) {
            /* static */ const flags1 = STATIC("flags1#tables-sizing-policies", imgui_js_18.ImGuiTableFlags.BordersV | imgui_js_18.ImGuiTableFlags.BordersOuterH | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.ContextMenuInBody);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendX", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.NoHostExtendX);
            PopStyleCompact();
            let sizing_policy_flags = [imgui_js_18.ImGuiTableFlags.SizingFixedFit, imgui_js_18.ImGuiTableFlags.SizingFixedSame, imgui_js_18.ImGuiTableFlags.SizingStretchProp, imgui_js_18.ImGuiTableFlags.SizingStretchSame];
            ...
            ;
            /* static */ const flags = STATIC("flags#tables-sizing-policies", imgui_js_18.ImGuiTableFlags.ScrollY | imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.Resizable);
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_PreciseWidths", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.PreciseWidths);
            ImGui.SameLine();
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags_ScrollX", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.ScrollX);
            ImGui.CheckboxFlags("ImGuiTableFlags_ScrollY", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.ScrollY);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoClip", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoClip);
            ImGui.PopItemWidth();
            ImGui.PopID();
            PopStyleCompact();
            if (ImGui.BeginTable("table2", column_count.value, flags.value, new imgui_js_25.ImVec2(0.0, TEXT_BASE_HEIGHT * 7))) {
                for (let cell = 0; cell < 10 * column_count.value; cell++) {
                    ...
                    switch (contents_type.value) {
                        ...
                        case ContentsType.CT_FillButton:
                            ImGui.Button(label, new imgui_js_25.ImVec2(-1.0, 0.0));
                            break;
        ...
        if (ImGui.TreeNode("Vertical scrolling, with clipping")) {
            ...
            /* static */ const flags = STATIC("flags#tables-vertical-scrolling", imgui_js_18.ImGuiTableFlags.ScrollY | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.BordersOuter | imgui_js_18.ImGuiTableFlags.BordersV | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_ScrollY", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.ScrollY);
            PopStyleCompact();
            // When using ScrollX or ScrollY we need to specify a size for our table container!
            // Otherwise by default the table will fit all available space, like a BeginChild() call.
            let outer_size = new imgui_js_25.ImVec2(0.0, TEXT_BASE_HEIGHT * 8);
            if (ImGui.BeginTable("table_scrolly", 3, flags.value, outer_size)) {
                ImGui.TableSetupScrollFreeze(0, 1); // Make top row always visible
                ImGui.TableSetupColumn("One", imgui_js_19.ImGuiTableColumnFlags.None);
                ImGui.TableSetupColumn("Two", imgui_js_19.ImGuiTableColumnFlags.None);
                ImGui.TableSetupColumn("Three", imgui_js_19.ImGuiTableColumnFlags.None);
                ImGui.TableHeadersRow();
                // Demonstrate using clipper for large vertical lists
                let clipper = new imgui_js_32.ImGuiListClipper();
                clipper.Begin(1000);
                while (clipper.Step()) {
...
"because the container window won't automatically extend vertically to fix contents (this may be improved in future versions)."`);
            /* static */ const flags = STATIC("flags#tables-horizontal-scrolling", imgui_js_18.ImGuiTableFlags.ScrollX | imgui_js_18.ImGuiTableFlags.ScrollY | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.BordersOuter | imgui_js_18.ImGuiTableFlags.BordersV | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable);
            ...
            if (ImGui.BeginTable("table_scrollx", 7, flags.value, outer_size)) {
                ...
                ImGui.TableSetupColumn("Line #", imgui_js_19.ImGuiTableColumnFlags.NoHide); // Make the first column not hideable to match our use of TableSetupScrollFreeze()
                ImGui.TableSetupColumn("One");
                ImGui.TableSetupColumn("Two");
                ImGui.TableSetupColumn("Three");
...
Without an explicit value, inner_width is == outer_size.x and therefore using Stretch columns + ScrollX together doesn't make sense.`);
            /* static */ const flags2 = STATIC("flags2#tables-horizontal-scrolling", imgui_js_18.ImGuiTableFlags.SizingStretchSame | imgui_js_18.ImGuiTableFlags.ScrollX | imgui_js_18.ImGuiTableFlags.ScrollY | imgui_js_18.ImGuiTableFlags.BordersOuter | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.ContextMenuInBody);
            /* static */ const inner_width = STATIC("inner_width#tables-horizontal-scrolling", 1000.0);
            PushStyleCompact();
            ImGui.PushID("flags3");
            ImGui.PushItemWidth(TEXT_BASE_WIDTH * 30);
            ImGui.CheckboxFlags("ImGuiTableFlags_ScrollX", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.ScrollX);
        ...
        if (ImGui.TreeNode("Columns flags")) {
            ...
            let column_flags = [
                imgui_js_19.ImGuiTableColumnFlags.DefaultSort, imgui_js_19.ImGuiTableColumnFlags.None, imgui_js_19.ImGuiTableColumnFlags.DefaultHide
            ...
            if (ImGui.BeginTable("table_columns_flags_checkboxes", column_count, imgui_js_18.ImGuiTableFlags.None)) {
                PushStyleCompact();
            ...
            let flags = imgui_js_18.ImGuiTableFlags.SizingFixedFit | imgui_js_18.ImGuiTableFlags.ScrollX | imgui_js_18.ImGuiTableFlags.ScrollY
                | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.BordersOuter | imgui_js_18.ImGuiTableFlags.BordersV
                | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable | imgui_js_18.ImGuiTableFlags.Sortable;
            let outer_size = new imgui_js_25.ImVec2(0.0, TEXT_BASE_HEIGHT * 9);
        ...
        if (ImGui.TreeNode("Columns widths")) {
            ...
            /* static */ const flags1 = STATIC("flags1#tables-columns-widths", imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.NoBordersInBodyUntilResize);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Resizable", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.Resizable);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoBordersInBodyUntilResize", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.NoBordersInBodyUntilResize);
            PopStyleCompact();
            if (ImGui.BeginTable("table1", 3, flags1.value)) {
                // We could also set ImGuiTableFlags_SizingFixedFit on the table and all columns will default to ImGuiTableColumnFlags_WidthFixed.
                ImGui.TableSetupColumn("one", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, 100.0); // Default to 100.0
                ImGui.TableSetupColumn("two", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, 200.0); // Default to 200.0
                ImGui.TableSetupColumn("three", imgui_js_19.ImGuiTableColumnFlags.WidthFixed); // Default to auto
                ImGui.TableHeadersRow();
                for (let row = 0; row < 4; row++) {
                    ImGui.TableNextRow();
                    for (let column = 0; column < 3; column++) {
            ...
            /* static */ const flags2 = STATIC("flags2#tables-columns-widths", imgui_js_18.ImGuiTableFlags.None);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_NoKeepColumnsVisible", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.NoKeepColumnsVisible);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersInnerV", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.BordersInnerV);
            ImGui.CheckboxFlags("ImGuiTableFlags_BordersOuterV", (value = flags2.value) => flags2.value = value, imgui_js_18.ImGuiTableFlags.BordersOuterV);
            PopStyleCompact();
            if (ImGui.BeginTable("table2", 4, flags2.value)) {
                // We could also set ImGuiTableFlags_SizingFixedFit on the table and all columns will default to ImGuiTableColumnFlags_WidthFixed.
                ImGui.TableSetupColumn("", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, 100.0);
                ImGui.TableSetupColumn("", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 15.0);
                ImGui.TableSetupColumn("", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 30.0);
                ImGui.TableSetupColumn("", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 15.0);
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
        ...
        if (ImGui.TreeNode("Nested tables")) {
            ...
            if (ImGui.BeginTable("table.nested1", 2, imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable)) {
                ...
                ImGui.Text("A0 Cell 0");
                {
                    ...
                    if (ImGui.BeginTable("table.nested2", 2, imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable)) {
                        ImGui.TableSetupColumn("B0");
                        ImGui.TableSetupColumn("B1");
                        ImGui.TableHeadersRow();
                        ImGui.TableNextRow(imgui_js_20.ImGuiTableRowFlags.None, rows_height);
                        ImGui.TableNextColumn();
        ...
        if (ImGui.TreeNode("Outer size")) {
            ...
            PushStyleCompact();
            /* static */ const flags = STATIC("flags#tables-row-height", imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.ContextMenuInBody | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.SizingFixedFit | imgui_js_18.ImGuiTableFlags.NoHostExtendX);
            ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendX", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoHostExtendX);
            ImGui.SameLine();
            HelpMarker("Make outer width auto-fit to columns, overriding outer_size.x value.\n\nOnly available when ScrollX/ScrollY are disabled and Stretch columns are not used.");
            ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendY", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoHostExtendY);
            ImGui.SameLine();
            ...
            PopStyleCompact();
            let outer_size = new imgui_js_25.ImVec2(0.0, TEXT_BASE_HEIGHT * 5.5);
            ...
            if (ImGui.BeginTable("table2", 3, imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.RowBg, new imgui_js_25.ImVec2(TEXT_BASE_WIDTH * 30, 0.0))) {
                for (let row = 0; row < 5; row++) {
                    ImGui.TableNextRow();
                    for (let column = 0; column < 3; column++) {
                        ImGui.TableNextColumn();
            ...
            ImGui.SameLine();
            if (ImGui.BeginTable("table3", 3, imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.RowBg, new imgui_js_25.ImVec2(TEXT_BASE_WIDTH * 30, 0.0))) {
                for (let row = 0; row < 3; row++) {
        ...
        if (ImGui.TreeNode("Background color")) {
            /* static */ const flags = STATIC("flags#tables-bg", imgui_js_18.ImGuiTableFlags.RowBg);
            ...
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_Borders", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Borders);
            ImGui.CheckboxFlags("ImGuiTableFlags_RowBg", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.RowBg);
            ImGui.SameLine();
            ...
            if (ImGui.BeginTable("table1", 5, flags.value)) {
                for (let row = 0; row < 6; row++) {
                    ...
                    if (row_bg_type.value != 0) {
                        let row_bg_color = ImGui.GetColorU32(row_bg_type.value == 1 ? new imgui_js_26.ImVec4(0.7, 0.3, 0.3, 0.65) : new imgui_js_26.ImVec4(0.2 + row * 0.1, 0.2, 0.2, 0.65)); // Flat or Gradient?
                        ImGui.TableSetBgColor(imgui_js_21.ImGuiTableBgTarget.RowBg0 + row_bg_target.value, row_bg_color);
                    }
                    // Fill cells
                    for (let column = 0; column < 5; column++) {
                        ...
                        if (row >= 1 && row <= 2 && column >= 1 && column <= 2 && cell_bg_type.value == 1) {
                            let cell_bg_color = ImGui.GetColorU32(new imgui_js_26.ImVec4(0.3, 0.3, 0.7, 0.65));
                            ImGui.TableSetBgColor(imgui_js_21.ImGuiTableBgTarget.CellBg, cell_bg_color);
                        }
                    }
                }
                ImGui.EndTable();
        ...
        if (ImGui.TreeNode("Tree view")) {
            /* static */ const flags = STATIC("flags#tables-tree-view", imgui_js_18.ImGuiTableFlags.BordersV | imgui_js_18.ImGuiTableFlags.BordersOuterH | imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.NoBordersInBody);
            if (ImGui.BeginTable("3ways", 3, flags.value)) {
                // The first column will use the default _WidthStretch when ScrollX is Off and _WidthFixed when ScrollX is On
                ImGui.TableSetupColumn("Name", imgui_js_19.ImGuiTableColumnFlags.NoHide);
                ImGui.TableSetupColumn("Size", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 12.0);
                ImGui.TableSetupColumn("Type", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, TEXT_BASE_WIDTH * 18.0);
                ImGui.TableHeadersRow();
                ...
                class MyTreeNode {
                    ...
                    static DisplayNode(node, all_nodes) {
                        ...
                        if (is_folder) {
                            let open = ImGui.TreeNodeEx(node.Name, imgui_js_15.ImGuiTreeNodeFlags.SpanFullWidth);
                            ImGui.TableNextColumn();
                            ImGui.TextDisabled("--");
                            ImGui.TableNextColumn();
                            ...
                            ImGui.TreeNodeEx(node.Name, imgui_js_15.ImGuiTreeNodeFlags.Leaf | imgui_js_15.ImGuiTreeNodeFlags.Bullet | imgui_js_15.ImGuiTreeNodeFlags.NoTreePushOnOpen | imgui_js_15.ImGuiTreeNodeFlags.SpanFullWidth);
                            ImGui.TableNextColumn();
        ...
        if (ImGui.TreeNode("Item width")) {
            ...
            if (ImGui.BeginTable("table_item_width", 3, imgui_js_18.ImGuiTableFlags.Borders)) {
        ...
        if (ImGui.TreeNode("Custom headers")) {
            ...
            if (ImGui.BeginTable("table_custom_headers", COLUMNS_COUNT, imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable)) {
                ...
                ImGui.TableNextRow(imgui_js_20.ImGuiTableRowFlags.Headers);
        ...
        if (ImGui.TreeNode("Context menus")) {
            ...
            /* static */ const flags1 = STATIC("flags1#tables-context-menus", imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable | imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.ContextMenuInBody);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_ContextMenuInBody", (value = flags1.value) => flags1.value = value, imgui_js_18.ImGuiTableFlags.ContextMenuInBody);
            PopStyleCompact();
            ...
            /* static */ const flags2 = STATIC("flags2#tables-context-menus", imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.SizingFixedFit | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable | imgui_js_18.ImGuiTableFlags.Borders);
            if (ImGui.BeginTable("table_context_menu_2", COLUMNS_COUNT, flags2.value)) {
                ...
                for (let column = 0; column < COLUMNS_COUNT + 1; column++) {
                    ImGui.PushID(column);
                    if (ImGui.TableGetColumnFlags(column) & imgui_js_19.ImGuiTableColumnFlags.IsHovered)
        ...
        if (ImGui.TreeNode("Synced instances")) {
            ...
            for (let n = 0; n < 3; n++) {
                ...
                let open = ImGui.CollapsingHeader(buf, imgui_js_15.ImGuiTreeNodeFlags.DefaultOpen);
                if (open && ImGui.BeginTable("Table", 3, imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable | imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.SizingFixedFit | imgui_js_18.ImGuiTableFlags.NoSavedSettings)) {
                    ImGui.TableSetupColumn("One");
                    ImGui.TableSetupColumn("Two");
                    ImGui.TableSetupColumn("Three");
                    ImGui.TableHeadersRow();
        ...
        if (ImGui.TreeNode("Sorting")) {
            // Options
            /* static */ const flags = STATIC("flags#tables-sorting", imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable | imgui_js_18.ImGuiTableFlags.Sortable | imgui_js_18.ImGuiTableFlags.SortMulti | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.BordersOuter | imgui_js_18.ImGuiTableFlags.BordersV | imgui_js_18.ImGuiTableFlags.NoBordersInBody | imgui_js_18.ImGuiTableFlags.ScrollY);
            PushStyleCompact();
            ImGui.CheckboxFlags("ImGuiTableFlags_SortMulti", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.SortMulti);
            ImGui.SameLine();
            ...
            ImGui.CheckboxFlags("ImGuiTableFlags_SortTristate", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.SortTristate);
            ImGui.SameLine();
            ...
            PopStyleCompact();
            if (ImGui.BeginTable("table_sorting", 4, flags.value, new imgui_js_25.ImVec2(0.0, TEXT_BASE_HEIGHT * 15), 0.0)) {
                // Declare columns
                ...
                ImGui.TableSetupColumn("ID", imgui_js_19.ImGuiTableColumnFlags.DefaultSort | imgui_js_19.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.ID);
                ImGui.TableSetupColumn("Name", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.Name);
                ImGui.TableSetupColumn("Action", imgui_js_19.ImGuiTableColumnFlags.NoSort | imgui_js_19.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.Action);
                ImGui.TableSetupColumn("Quantity", imgui_js_19.ImGuiTableColumnFlags.PreferSortDescending | imgui_js_19.ImGuiTableColumnFlags.WidthStretch, 0.0, MyItemColumnID.Quantity);
                        ...
                        if (table_sort_items.value.length > 1)
                            table_sort_items.value.sort((a, b) => {
                                ...
                                for (let n = 0; n < sort_specs.SpecsCount; n++) {
                                    ...
                                    if (delta > 0)
                                        return sort_spec.SortDirection == imgui_js_17.ImGuiSortDirection.Ascending ? 1 : -1;
                                    if (delta < 0)
                                        return sort_spec.SortDirection == imgui_js_17.ImGuiSortDirection.Ascending ? -1 : 1;
                                }
                ...
                // Demonstrate using clipper for large vertical lists
                const clipper = new imgui_js_32.ImGuiListClipper();
        ...
        if (ImGui.TreeNode("Advanced")) {
            /* static */ const flags = STATIC("flags#tables-sorting-advanced", imgui_js_18.ImGuiTableFlags.Resizable | imgui_js_18.ImGuiTableFlags.Reorderable | imgui_js_18.ImGuiTableFlags.Hideable | imgui_js_18.ImGuiTableFlags.Sortable | imgui_js_18.ImGuiTableFlags.SortMulti | imgui_js_18.ImGuiTableFlags.RowBg | imgui_js_18.ImGuiTableFlags.Borders | imgui_js_18.ImGuiTableFlags.NoBordersInBody | imgui_js_18.ImGuiTableFlags.ScrollX | imgui_js_18.ImGuiTableFlags.ScrollY | imgui_js_18.ImGuiTableFlags.SizingFixedFit);
            let ContentsType;
            (function (ContentsType) {
            ...
            const outer_size_value = STATIC("outer_size_value#tables-sorting-advanced", new imgui_js_25.ImVec2(0.0, TEXT_BASE_HEIGHT * 12));
            ...
            if (ImGui.TreeNode("Options")) {
                ...
                if (ImGui.TreeNodeEx("Features:", imgui_js_15.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.Resizable", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Resizable);
                    ImGui.CheckboxFlags("ImGuiTableFlags.Reorderable", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Reorderable);
                    ImGui.CheckboxFlags("ImGuiTableFlags.Hideable", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Hideable);
                    ImGui.CheckboxFlags("ImGuiTableFlags.Sortable", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.Sortable);
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoSavedSettings", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoSavedSettings);
                    ImGui.CheckboxFlags("ImGuiTableFlags.ContextMenuInBody", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.ContextMenuInBody);
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Decorations:", imgui_js_15.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.RowBg", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.RowBg);
                    ImGui.CheckboxFlags("ImGuiTableFlags.BordersV", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersV);
                    ImGui.CheckboxFlags("ImGuiTableFlags.BordersOuterV", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersOuterV);
                    ImGui.CheckboxFlags("ImGuiTableFlags.BordersInnerV", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.BordersInnerV);
                    ...
                    ImGui.SameLine();
                    HelpMarker("Disable vertical borders in columns Body (borders will always appears in Headers");
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoBordersInBodyUntilResize", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoBordersInBodyUntilResize);
                    ImGui.SameLine();
                    HelpMarker("Disable vertical borders in columns Body until hovered for resize (borders will always appears in Headers)");
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Sizing:", imgui_js_15.ImGuiTreeNodeFlags.DefaultOpen)) {
                    EditTableSizingFlags(flags);
                    ImGui.SameLine();
                    ...
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoHostExtendX", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoHostExtendX);
                    ImGui.SameLine();
                    HelpMarker("Make outer width auto-fit to columns, overriding outer_size.x value.\n\nOnly available when ScrollX/ScrollY are disabled and Stretch columns are not used.");
                    ImGui.CheckboxFlags("ImGuiTableFlags_NoHostExtendY", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoHostExtendY);
                    ImGui.SameLine();
                    HelpMarker("Make outer height stop exactly at outer_size.y (prevent auto-extending table past the limit).\n\nOnly available when ScrollX/ScrollY are disabled. Data below the limit will be clipped and not visible.");
                    ImGui.CheckboxFlags("ImGuiTableFlags_NoKeepColumnsVisible", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoKeepColumnsVisible);
                    ImGui.SameLine();
                ...
                if (ImGui.TreeNodeEx("Padding:", imgui_js_15.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.PadOuterX", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.PadOuterX);
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoPadOuterX", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoPadOuterX);
                    ImGui.CheckboxFlags("ImGuiTableFlags.NoPadInnerX", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.NoPadInnerX);
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Scrolling:", imgui_js_15.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.ScrollX", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.ScrollX);
                    ImGui.SameLine();
                    ImGui.SetNextItemWidth(ImGui.GetFrameHeight());
                    ImGui.DragInt("freeze_cols", (value = freeze_cols.value) => freeze_cols.value = value, 0.2, 0, 9);
                    ImGui.CheckboxFlags("ImGuiTableFlags.ScrollY", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.ScrollY);
                    ImGui.SameLine();
                    ImGui.SetNextItemWidth(ImGui.GetFrameHeight());
                    ImGui.DragInt("freeze_rows", (value = freeze_rows.value) => freeze_rows.value = value, 0.2, 0, 9);
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Sorting:", imgui_js_15.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.CheckboxFlags("ImGuiTableFlags.SortMulti", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.SortMulti);
                    ImGui.SameLine();
                    HelpMarker("When sorting is enabled: hold shift when clicking headers to sort on multiple column. TableGetSortSpecs() may return specs where (SpecsCount > 1).");
                    ImGui.CheckboxFlags("ImGuiTableFlags.SortTristate", (value = flags.value) => flags.value = value, imgui_js_18.ImGuiTableFlags.SortTristate);
                    ImGui.SameLine();
                    HelpMarker("When sorting is enabled: allow no sorting, disable default sorting. TableGetSortSpecs() may return specs where (SpecsCount == 0).");
                    ImGui.TreePop();
                }
                if (ImGui.TreeNodeEx("Other:", imgui_js_15.ImGuiTreeNodeFlags.DefaultOpen)) {
                    ImGui.Checkbox("show_headers", (value = show_headers.value) => show_headers.value = value);
            ...
            // for debug display
            let table_scroll_cur = new imgui_js_25.ImVec2(0.0, 0.0);
            let table_scroll_max = new imgui_js_25.ImVec2(0.0, 0.0);
            let table_draw_list = null; // "
            const inner_width_to_use = (flags.value & imgui_js_18.ImGuiTableFlags.ScrollX) ? inner_width_with_scroll.value : 0.0;
            if (ImGui.BeginTable("table_advanced", 6, flags.value, outer_size_enabled.value ? outer_size_value.value : new imgui_js_25.ImVec2(0, 0), inner_width_to_use)) {
                // Declare columns
                // We use the "user_id" parameter of TableSetupColumn() to specify a user id that will be stored in the sort specifications.
                // This is so our sort function can identify a column given our own identifier. We could also identify them based on their index!
                ImGui.TableSetupColumn("ID", imgui_js_19.ImGuiTableColumnFlags.DefaultSort | imgui_js_19.ImGuiTableColumnFlags.WidthFixed | imgui_js_19.ImGuiTableColumnFlags.NoHide, 0.0, MyItemColumnID.ID);
                ImGui.TableSetupColumn("Name", imgui_js_19.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.Name);
                ImGui.TableSetupColumn("Action", imgui_js_19.ImGuiTableColumnFlags.NoSort | imgui_js_19.ImGuiTableColumnFlags.WidthFixed, 0.0, MyItemColumnID.Action);
                ImGui.TableSetupColumn("Quantity", imgui_js_19.ImGuiTableColumnFlags.PreferSortDescending, 0.0, MyItemColumnID.Quantity);
                ImGui.TableSetupColumn("Description", (flags.value & imgui_js_18.ImGuiTableFlags.NoHostExtendX) ? 0 : imgui_js_19.ImGuiTableColumnFlags.WidthStretch, 0.0, MyItemColumnID.Description);
                ImGui.TableSetupColumn("Hidden", imgui_js_19.ImGuiTableColumnFlags.DefaultHide | imgui_js_19.ImGuiTableColumnFlags.NoSort);
                ImGui.TableSetupScrollFreeze(freeze_cols.value, freeze_rows.value);
                // Sort our data if sort specs have been changed!
                ...
                const sorts_specs_using_quantity = (ImGui.TableGetColumnFlags(3) & imgui_js_19.ImGuiTableColumnFlags.IsSorted) != 0;
                // Show headers
                if (show_headers.value)
                    ImGui.TableHeadersRow();
                // Show data
                ...
                while (clipper.Step()) {
                    for (let row_n = clipper.DisplayStart; row_n < clipper.DisplayEnd; row_n++) {
                        ...
                        ImGui.PushID(item.ID);
                        ImGui.TableNextRow(imgui_js_20.ImGuiTableRowFlags.None, row_min_height.value);
                        ImGui.TableNextColumn();
                        ...
                        if (contents_type.value == ContentsType.Text)
                            ...
                            const selectable_flags = (contents_type.value == ContentsType.SelectableSpanRow) ? imgui_js_13.ImGuiSelectableFlags.SpanAllColumns | imgui_js_13.ImGuiSelectableFlags.AllowItemOverlap : imgui_js_13.ImGuiSelectableFlags.None;
                            if (ImGui.Selectable(label, item_is_selected, selectable_flags, new imgui_js_25.ImVec2(0, row_min_height.value))) {
                                if (ImGui.GetIO().KeyCtrl) {
                                    if (item_is_selected)
                ...
                table_scroll_cur = new imgui_js_25.ImVec2(ImGui.GetScrollX(), ImGui.GetScrollY());
                table_scroll_max = new imgui_js_25.ImVec2(ImGui.GetScrollMaxX(), ImGui.GetScrollMaxY());
    ...
    function ShowDemoWindowMisc() {
        if (ImGui.CollapsingHeader("Filtering")) {
            /* static */ const filter = STATIC("filter#1864", new imgui_js_30.ImGuiTextFilter());
        ...
        if (ImGui.CollapsingHeader("Inputs, Navigation & Focus")) {
            ...
            if (ImGui.TreeNode("Mouse cursors")) {
                ...
                imgui_js_2.IM_ASSERT(imgui_js_3.IM_ARRAYSIZE(mouse_cursors_names) === imgui_js_12.ImGuiMouseCursor.COUNT);
                ...
                for (let i = 0; i < imgui_js_12.ImGuiMouseCursor.COUNT; i++) {
    ...
    function ShowAboutWindow(p_open) {
        if (!ImGui.Begin("About Dear ImGui", p_open, imgui_js_16.ImGuiWindowFlags.AlwaysAutoResize)) {
            ImGui.End();
            return;
        }
    ...
    function ShowStyleEditor(ref = null) {
        ...
        const style = ImGui.GetStyle();
        /* static */ const ref_saved_style = STATIC("ref_saved_style", new imgui_js_29.ImGuiStyle());
        ...
        ImGui.Separator();
        if (ImGui.BeginTabBar("##tabs", imgui_js_22.ImGuiTabBarFlags.None)) {
            ...
            if (ImGui.BeginTabItem("Colors")) {
                ...
                /* static */ const filter = STATIC("filter#2223", new imgui_js_30.ImGuiTextFilter());
                ...
                ImGui.BeginChild("#colors", new imgui_js_25.ImVec2(0, 300), true, imgui_js_16.ImGuiWindowFlags.AlwaysVerticalScrollbar | imgui_js_16.ImGuiWindowFlags.AlwaysHorizontalScrollbar | imgui_js_16.ImGuiWindowFlags.NavFlattened);
            ...
            if (ImGui.BeginTabItem("Fonts")) {
                        ...
                        if (ImGui.TreeNode("Glyphs", `Glyphs (${font.Glyphs.Size})`)) {
                            ...
                            for (let base = 0; base < 0x10000; base += 256) {
                                ...
                                if (count > 0 && ImGui.TreeNode(base, `U+${format_number_hex(base, 4).toUpperCase()}..U+${(format_number_hex(base + 255, 4).toUpperCase())} (${count} ${count > 1 ? "glyphs" : "glyph"})`)) {
                                    ...
                                    for (let n = 0; n < 256; n++) {
                                        const cell_p1 = new imgui_js_25.ImVec2(base_pos.x + (n % 16) * (cell_size + cell_spacing), base_pos.y + (0 | (n / 16)) * (cell_size + cell_spacing));
                                        const cell_p2 = new imgui_js_25.ImVec2(cell_p1.x + cell_size, cell_p1.y + cell_size);
                                        const glyph = font.FindGlyphNoFallback((base + n));
                                        draw_list.AddRect(cell_p1, cell_p2, glyph ? imgui_js_27.IM_COL32(255, 255, 255, 100) : imgui_js_27.IM_COL32(255, 255, 255, 50));
                                        if (glyph)
                                        ...
                                        if (glyph && ImGui.IsWindowHovered() && ImGui.IsMouseHoveringRect(cell_p1, cell_p2)) {
                                            ...
                                            ImGui.Separator();
                                            ImGui.Image(ImGui.GetIO().Fonts.TexID, new imgui_js_25.ImVec2(8 * (glyph.X1 - glyph.X0), 8 * (glyph.Y1 - glyph.Y0)), new imgui_js_25.ImVec2(glyph.U0, glyph.V0), new imgui_js_25.ImVec2(glyph.U1, glyph.V1), new imgui_js_28.ImColor(255, 255, 255, 255).toImVec4(), new imgui_js_28.ImColor(255, 255, 255, 128).toImVec4());
                                            ImGui.SameLine();
                                            ImGui.BeginGroup();
                                    ...
                                    ImGui.Dummy(new imgui_js_25.ImVec2((cell_size + cell_spacing) * 16, (cell_size + cell_spacing) * 16));
                                    ImGui.TreePop();
                                }
                            }
                            ImGui.TreePop();
                ...
                if (ImGui.TreeNode("Atlas texture", `Atlas texture (${atlas.TexWidth}x${atlas.TexHeight} pixels)`)) {
                    const tint_col = new imgui_js_26.ImVec4(1.0, 1.0, 1.0, 1.0);
                    const border_col = new imgui_js_26.ImVec4(1.0, 1.0, 1.0, 0.5);
                    ImGui.Image(atlas.TexID, new imgui_js_25.ImVec2(atlas.TexWidth, atlas.TexHeight), new imgui_js_25.ImVec2(0, 0), new imgui_js_25.ImVec2(1, 1), tint_col, border_col);
                    ImGui.TreePop();
                }
    ...
    function ShowExampleMenuFile() {
        ...
        if (ImGui.BeginMenu("Options")) {
            ...
            ImGui.BeginChild("child", new imgui_js_25.ImVec2(0, 60), true);
            for (let i = 0; i < 10; i++)
        ...
        if (ImGui.BeginMenu("Colors")) {
            ...
            for (let i = 0; i < imgui_js_5.ImGuiCol.COUNT; i++) {
                ...
                ImGui.GetWindowDrawList().AddRectFilled(p, new imgui_js_25.ImVec2(p.x + sz, p.y + sz), ImGui.GetColorU32(i));
                ImGui.Dummy(new imgui_js_25.ImVec2(sz, sz));
                ImGui.SameLine();
                ImGui.MenuItem(name);
            }
            ImGui.EndMenu();
    ...
    function ShowExampleAppLog(p_open) {
        ...
        ImGui.SetNextWindowSize(new imgui_js_25.ImVec2(500, 400), imgui_js_7.ImGuiCond.FirstUseEver);
    ...
    function ShowExampleAppLayout(p_open) {
        ImGui.SetNextWindowSize(new imgui_js_25.ImVec2(500, 440), imgui_js_7.ImGuiCond.FirstUseEver);
        if (ImGui.Begin("Example: Simple Layout", p_open, imgui_js_16.ImGuiWindowFlags.MenuBar)) {
            if (ImGui.BeginMenuBar()) {
                if (ImGui.BeginMenu("File")) {
                    if (ImGui.MenuItem("Close"))
                        p_open(false);
            ...
            ImGui.BeginChild("left pane", new imgui_js_25.ImVec2(150, 0), true);
            for (let i = 0; i < 100; i++) {
            ...
            // right
            ImGui.BeginGroup();
            ImGui.BeginChild("item view", new imgui_js_25.ImVec2(0, -ImGui.GetFrameHeightWithSpacing())); // Leave room for 1 line below us
    ...
    function ShowExampleAppPropertyEditor(p_open) {
        ImGui.SetNextWindowSize(new imgui_js_25.ImVec2(430, 450), imgui_js_7.ImGuiCond.FirstUseEver);
        ...
        ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.FramePadding, new imgui_js_25.ImVec2(2, 2));
        ImGui.Columns(2);
        ImGui.Separator();
        class funcs {
                ...
                if (node_open) {
                    ...
                    for (let i = 0; i < 8; i++) {
                        ...
                        if (i < 2) {
                            ...
                            ImGui.TreeNodeEx("Field", imgui_js_15.ImGuiTreeNodeFlags.Leaf | imgui_js_15.ImGuiTreeNodeFlags.NoTreePushOnOpen | imgui_js_15.ImGuiTreeNodeFlags.Bullet, `Field_${i}`);
                            ImGui.NextColumn();
                            ImGui.SetNextItemWidth(-1);
    ...
    function ShowExampleAppLongText(p_open) {
        ImGui.SetNextWindowSize(new imgui_js_25.ImVec2(520, 600), imgui_js_7.ImGuiCond.FirstUseEver);
        if (!ImGui.Begin("Example: Long text display", p_open)) {
            ImGui.End();
            return;
        }
        /* static */ const test_type = STATIC("test_type", 0);
        /* static */ const log = STATIC("log#3217", new imgui_js_31.ImGuiTextBuffer());
            ...
            case 1:
                {
                    ...
                    ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.ItemSpacing, new imgui_js_25.ImVec2(0, 0));
                    const clipper = new imgui_js_32.ImGuiListClipper();
    ...
    function ShowExampleAppConstrainedResize(p_open) {
        ...
        if (type.value === 0)
            ImGui.SetNextWindowSizeConstraints(new imgui_js_25.ImVec2(-1, 0), new imgui_js_25.ImVec2(-1, Number.MAX_VALUE)); // Vertical only
        if (type.value === 1)
            ImGui.SetNextWindowSizeConstraints(new imgui_js_25.ImVec2(0, -1), new imgui_js_25.ImVec2(Number.MAX_VALUE, -1)); // Horizontal only
        if (type.value === 2)
            ImGui.SetNextWindowSizeConstraints(new imgui_js_25.ImVec2(100, 100), new imgui_js_25.ImVec2(Number.MAX_VALUE, Number.MAX_VALUE)); // Width > 100, Height > 100
        if (type.value === 3)
            ImGui.SetNextWindowSizeConstraints(new imgui_js_25.ImVec2(400, -1), new imgui_js_25.ImVec2(500, -1)); // Width 400-500
        if (type.value === 4)
            ImGui.SetNextWindowSizeConstraints(new imgui_js_25.ImVec2(-1, 400), new imgui_js_25.ImVec2(-1, 500)); // Height 400-500
        if (type.value === 5)
            ImGui.SetNextWindowSizeConstraints(new imgui_js_25.ImVec2(0, 0), new imgui_js_25.ImVec2(Number.MAX_VALUE, Number.MAX_VALUE), CustomConstraints.Square); // Always Square
        if (type.value === 6)
            ImGui.SetNextWindowSizeConstraints(new imgui_js_25.ImVec2(0, 0), new imgui_js_25.ImVec2(Number.MAX_VALUE, Number.MAX_VALUE), CustomConstraints.Step, 100); // Fixed Step
        const flags = auto_resize.value ? imgui_js_16.ImGuiWindowFlags.AlwaysAutoResize : 0;
        if (ImGui.Begin("Example: Constrained Resize", p_open, flags)) {
            ...
            if (ImGui.Button("200x200")) {
                ImGui.SetWindowSize(new imgui_js_25.ImVec2(200, 200));
            }
            ImGui.SameLine();
            if (ImGui.Button("500x500")) {
                ImGui.SetWindowSize(new imgui_js_25.ImVec2(500, 500));
            }
            ImGui.SameLine();
            if (ImGui.Button("800x200")) {
                ImGui.SetWindowSize(new imgui_js_25.ImVec2(800, 200));
            }
    ...
    function ShowExampleAppSimpleOverlay(p_open) {
        ...
        const io = ImGui.GetIO();
        if (corner.value !== -1) {
            const window_pos = new imgui_js_25.ImVec2((corner.value & 1) ? io.DisplaySize.x - DISTANCE : DISTANCE, (corner.value & 2) ? io.DisplaySize.y - DISTANCE : DISTANCE);
            const window_pos_pivot = new imgui_js_25.ImVec2((corner.value & 1) ? 1.0 : 0.0, (corner.value & 2) ? 1.0 : 0.0);
        ...
        if (ImGui.Begin("Example: Simple overlay", p_open, (corner.value !== -1 ? imgui_js_16.ImGuiWindowFlags.NoMove : 0) | imgui_js_16.ImGuiWindowFlags.NoDecoration | imgui_js_16.ImGuiWindowFlags.AlwaysAutoResize | imgui_js_16.ImGuiWindowFlags.NoSavedSettings)) {
    ...
    function ShowExampleAppWindowTitles(p_open) {
        ...
        ImGui.SetNextWindowPos(new imgui_js_25.ImVec2(100, 100), imgui_js_7.ImGuiCond.FirstUseEver);
        ImGui.Begin("Same title as another window##1");
        ImGui.Text("This is window 1.\nMy title is the same as window 2, but my identifier is unique.");
        ImGui.End();
        ImGui.SetNextWindowPos(new imgui_js_25.ImVec2(100, 200), imgui_js_7.ImGuiCond.FirstUseEver);
        ...
        ImGui.SetNextWindowPos(new imgui_js_25.ImVec2(100, 300), imgui_js_7.ImGuiCond.FirstUseEver);
        ImGui.Begin(buf);
    ...
    function ShowExampleAppCustomRendering(p_open) {
        ImGui.SetNextWindowSize(new imgui_js_25.ImVec2(350, 560), imgui_js_7.ImGuiCond.FirstUseEver);
        ...
        if (ImGui.BeginTabBar("##TabBar")) {
            ...
            if (ImGui.BeginTabItem("Primitives")) {
                ...
                /* static */ const col = STATIC("color#2583", new imgui_js_26.ImVec4(1.0, 1.0, 0.4, 1.0));
                ...
                {
                    const p = ImGui.GetCursorScreenPos();
                    const col32 = imgui_js_27.IM_COL32(col.value.x * 255, col.value.y * 255, col.value.z * 255, col.value.w * 255);
                    let x = p.x + 4.0, y = p.y + 4.0;
                    const spacing = 8.0;
                    for (let n = 0; n < 2; n++) {
                        const curr_thickness = (n === 0) ? 1.0 : thickness.value;
                        draw_list.AddCircle(new imgui_js_25.ImVec2(x + sz.value * 0.5, y + sz.value * 0.5), sz.value * 0.5, col32, 20, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddRect(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value), col32, 0.0, imgui_js_23.ImDrawCornerFlags.All, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddRect(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value), col32, 10.0, imgui_js_23.ImDrawCornerFlags.All, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddRect(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value), col32, 10.0, imgui_js_23.ImDrawCornerFlags.TopLeft | imgui_js_23.ImDrawCornerFlags.BotRight, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddTriangle(new imgui_js_25.ImVec2(x + sz.value * 0.5, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value - 0.5), new imgui_js_25.ImVec2(x, y + sz.value - 0.5), col32, curr_thickness);
                        x += sz.value + spacing;
                        draw_list.AddLine(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y), col32, curr_thickness);
                        x += sz.value + spacing; // Horizontal line (note: drawing a filled rectangle will be faster!)
                        draw_list.AddLine(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x, y + sz.value), col32, curr_thickness);
                        x += spacing; // Vertical line (note: drawing a filled rectangle will be faster!)
                        draw_list.AddLine(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value), col32, curr_thickness);
                        x += sz.value + spacing; // Diagonal line
                        draw_list.AddBezierCubic(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value * 1.3, y + sz.value * 0.3), new imgui_js_25.ImVec2(x + sz.value - sz.value * 1.3, y + sz.value - sz.value * 0.3), new imgui_js_25.ImVec2(x + sz.value, y + sz.value), col32, curr_thickness);
                        x = p.x + 4;
                        y += sz.value + spacing;
                    }
                    draw_list.AddCircleFilled(new imgui_js_25.ImVec2(x + sz.value * 0.5, y + sz.value * 0.5), sz.value * 0.5, col32, 32);
                    x += sz.value + spacing;
                    draw_list.AddRectFilled(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value), col32);
                    x += sz.value + spacing;
                    draw_list.AddRectFilled(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value), col32, 10.0);
                    x += sz.value + spacing;
                    draw_list.AddRectFilled(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value), col32, 10.0, imgui_js_23.ImDrawCornerFlags.TopLeft | imgui_js_23.ImDrawCornerFlags.BotRight);
                    x += sz.value + spacing;
                    draw_list.AddTriangleFilled(new imgui_js_25.ImVec2(x + sz.value * 0.5, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value - 0.5), new imgui_js_25.ImVec2(x, y + sz.value - 0.5), col32);
                    x += sz.value + spacing;
                    draw_list.AddRectFilled(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y + thickness.value), col32);
                    x += sz.value + spacing; // Horizontal line (faster than AddLine, but only handle integer thickness)
                    draw_list.AddRectFilled(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + thickness.value, y + sz.value), col32);
                    x += spacing + spacing; // Vertical line (faster than AddLine, but only handle integer thickness)
                    draw_list.AddRectFilled(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + 1, y + 1), col32);
                    x += sz.value; // Pixel (faster than AddLine)
                    draw_list.AddRectFilledMultiColor(new imgui_js_25.ImVec2(x, y), new imgui_js_25.ImVec2(x + sz.value, y + sz.value), imgui_js_27.IM_COL32(0, 0, 0), imgui_js_27.IM_COL32(255, 0, 0), imgui_js_27.IM_COL32(255, 255, 0), imgui_js_27.IM_COL32(0, 255, 0));
                    ImGui.Dummy(new imgui_js_25.ImVec2((sz.value + spacing) * 8, (sz.value + spacing) * 3));
                }
                ImGui.EndTabItem();
            }
            if (ImGui.BeginTabItem("Canvas")) {
                /* static */ const points = STATIC("points", new imgui_js_24.ImVector());
                ...
                if (canvas_size.y < 50.0)
                    canvas_size.y = 50.0;
                draw_list.AddRectFilledMultiColor(canvas_pos, new imgui_js_25.ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), imgui_js_27.IM_COL32(50, 50, 50), imgui_js_27.IM_COL32(50, 50, 60), imgui_js_27.IM_COL32(60, 60, 70), imgui_js_27.IM_COL32(50, 50, 60));
                draw_list.AddRect(canvas_pos, new imgui_js_25.ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), imgui_js_27.IM_COL32(255, 255, 255));
                let adding_preview = false;
                ImGui.InvisibleButton("canvas", canvas_size);
                const mouse_pos_in_canvas = new imgui_js_25.ImVec2(ImGui.GetIO().MousePos.x - canvas_pos.x, ImGui.GetIO().MousePos.y - canvas_pos.y);
                ...
                draw_list.PushClipRect(canvas_pos, new imgui_js_25.ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), true); // clip lines within the canvas (if we resize it, etc.)
                for (let i = 0; i < points.value.Size - 1; i += 2)
                    draw_list.AddLine(new imgui_js_25.ImVec2(canvas_pos.x + points.value.Data[i].x, canvas_pos.y + points.value.Data[i].y), new imgui_js_25.ImVec2(canvas_pos.x + points.value.Data[i + 1].x, canvas_pos.y + points.value.Data[i + 1].y), imgui_js_27.IM_COL32(255, 255, 0, 255), 2.0);
            ...
            if (ImGui.BeginTabItem("BG/FG draw lists")) {
                ...
                const window_center = new imgui_js_25.ImVec2(window_pos.x + window_size.x * 0.5, window_pos.y + window_size.y * 0.5);
                if (draw_bg.value)
                    ImGui.GetBackgroundDrawList().AddCircle(window_center, window_size.x * 0.6, imgui_js_27.IM_COL32(255, 0, 0, 200), 32, 10 + 4);
                if (draw_fg.value)
                    ImGui.GetForegroundDrawList().AddCircle(window_center, window_size.y * 0.6, imgui_js_27.IM_COL32(0, 255, 0, 200), 32, 10);
                ImGui.EndTabItem();
            }
            ImGui.EndTabBar();
        }
    ...
    function ShowExampleAppDockspace(p_open) {
        ...
        let window_flags = imgui_js_16.ImGuiWindowFlags.MenuBar | imgui_js_16.ImGuiWindowFlags.NoDocking;
        if (opt_fullscreen.value) {
            ...
            ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.WindowRounding, 0.0);
            ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.WindowBorderSize, 0.0);
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoTitleBar | imgui_js_16.ImGuiWindowFlags.NoCollapse | imgui_js_16.ImGuiWindowFlags.NoResize | imgui_js_16.ImGuiWindowFlags.NoMove;
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoBringToFrontOnFocus | imgui_js_16.ImGuiWindowFlags.NoNavFocus;
        }
        else {
        ...
        if (dockspace_flags.value & imgui_js_9.ImGuiDockNodeFlags.PassthruCentralNode)
            window_flags |= imgui_js_16.ImGuiWindowFlags.NoBackground;
        ...
        if (!opt_padding.value)
            ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.WindowPadding, new imgui_js_25.ImVec2(0.0, 0.0));
        ...
        if (ImGui.BeginMenuBar()) {
            if (ImGui.BeginMenu("Options")) {
                ...
                if (ImGui.MenuItem("Flag: PassthruCentralNode", "", (dockspace_flags.value & imgui_js_9.ImGuiDockNodeFlags.PassthruCentralNode) != 0, opt_fullscreen.value)) {
                    dockspace_flags.value ^= imgui_js_9.ImGuiDockNodeFlags.PassthruCentralNode;
                }
                ImGui.Separator();
    ...
    function ShowExampleAppDocuments(p_open) {
        ...
        if (!ImGui.Begin("Example: Documents", p_open, imgui_js_16.ImGuiWindowFlags.MenuBar)) {
            ImGui.End();
            return;
        }
        // // Menu
        ...
        if (ImGui.TreeNode("0001: Renderer: Large Mesh Support")) {
            const draw_list = ImGui.GetWindowDrawList();
            {
                ...
                for (let n = 0; n < vtx_count.value / 4; n++) {
                    ...
                    const col = imgui_js_27.IM_COL32(((n * 17) & 255), ((n * 59) & 255), ((n * 83) & 255), 255);
                    // draw_list->AddRectFilled(ImVec2(p.x + off_x, p.y + off_y), ImVec2(p.x + off_x + 50, p.y + off_y + 50), col);
                    draw_list.AddRectFilled(new imgui_js_25.ImVec2(p.x + off_x, p.y + off_y), new imgui_js_25.ImVec2(p.x + off_x + 50, p.y + off_y + 50), col);
                }
                ImGui.Dummy(new imgui_js_25.ImVec2(300 + 50, 100 + 50));
            ...
            }
            {
                ...
                for (let n = 0; n < vtx_count.value / (10 * 4); n++) {
                    ...
                    draw_list.AddText(new imgui_js_25.ImVec2(p.x + off_x, p.y + off_y), col, "ABCDEFGHIJ");
                }
                ImGui.Dummy(new imgui_js_25.ImVec2(300 + 50, 100 + 20));
                // ImGui.Text("VtxBuffer.Size = %d", draw_list->VtxBuffer.Size);
                ImGui.Text(`VtxBuffer = ${draw_list.VtxBuffer.length}`);
            }
            ImGui.TreePop();
        ...
        execute: function () {
            ...
            ExampleAppConsole = class ExampleAppConsole {
                constructor() {
                    ...
                    // ImVector<char*>       Items;
                    this.Items = new imgui_js_24.ImVector();
                    // ImVector<const char*> Commands;
                    this.Commands = new imgui_js_24.ImVector();
                    // ImVector<char*>       History;
                    this.History = new imgui_js_24.ImVector();
                    // int                   HistoryPos;    // -1: new line, 0..History.Size-1 browsing history.
                    this.HistoryPos = -1;
                    // ImGuiTextFilter       Filter;
                    this.Filter = new imgui_js_30.ImGuiTextFilter();
                    // bool                  AutoScroll;
                ...
                Draw(title, p_open) {
                    ...
                    ImGui.BeginChild("ScrollingRegion", new imgui_js_25.ImVec2(0, -footer_height_to_reserve), false, imgui_js_16.ImGuiWindowFlags.HorizontalScrollbar); // Leave room for 1 separator + 1 InputText
                    if (ImGui.BeginPopupContextWindow()) {
                    ...
                    ImGui.PushStyleVar(imgui_js_14.ImGuiStyleVar.ItemSpacing, new imgui_js_25.ImVec2(4, 1)); // Tighten spacing
                    if (copy_to_clipboard)
                        ImGui.LogToClipboard();
                    for (let i = 0; i < this.Items.Size; i++) {
                        ...
                        if (/\[error\]/.test(item)) {
                            ImGui.PushStyleColor(imgui_js_5.ImGuiCol.Text, new imgui_js_26.ImVec4(1.0, 0.4, 0.4, 1.0));
                            pop_color = true;
                        }
                        // else if (strncmp(item, "# ", 2) == 0)   { ImGui.PushStyleColor(ImGuiCol_Text, new ImVec4(1.0f, 0.8f, 0.6f, 1.0f)); pop_color = true; }
                        else if (/^# /.test(item)) {
                            ImGui.PushStyleColor(imgui_js_5.ImGuiCol.Text, new imgui_js_26.ImVec4(1.0, 0.8, 0.6, 1.0));
                            pop_color = true;
                        }
                    ...
                    if (ImGui.InputText("Input", this.InputBuf, imgui_js_3.IM_ARRAYSIZE(this.InputBuf), imgui_js_11.ImGuiInputTextFlags.EnterReturnsTrue | imgui_js_11.ImGuiInputTextFlags.CallbackCompletion | imgui_js_11.ImGuiInputTextFlags.CallbackHistory, ExampleAppConsole.TextEditCallbackStub, this)) {
                        // char* s = InputBuf;
                        // Strtrim(s);
                        // if (s[0])
                        //     ExecCommand(s);
                ...
                TextEditCallback(data) {
                    ...
                    switch (data.EventFlag) {
                        case imgui_js_11.ImGuiInputTextFlags.CallbackCompletion:
                            {
                        ...
                        case imgui_js_11.ImGuiInputTextFlags.CallbackHistory:
                            {
                                // Example of HISTORY
            ...
            ExampleAppLog = class ExampleAppLog {
                constructor() {
                    // ImGuiTextBuffer     Buf;
                    this.Buf = new imgui_js_31.ImGuiTextBuffer();
                    // ImGuiTextFilter     Filter;
                    this.Filter = new imgui_js_30.ImGuiTextFilter();
                    // ImVector<int>       LineOffsets;        // Index to lines offset. We maintain this with AddLog() calls, allowing us to have a random access on lines
                    this.LineOffsets = new imgui_js_24.ImVector();
                    // bool                AutoScroll;
                    this.AutoScroll = true;
                    // bool                ScrollToBottom;
                    this.ScrollToBottom = false;
                ...
                Draw(title, p_open) {
                    ...
                    ImGui.Separator();
                    ImGui.BeginChild("scrolling", new imgui_js_25.ImVec2(0, 0), false, imgui_js_16.ImGuiWindowFlags.HorizontalScrollbar);
                    if (clear)
                        this.Clear();
                    if (copy)
                        ImGui.LogToClipboard();
